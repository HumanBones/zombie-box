...
func _physics_process(delta: float) ->void:
	...
	var distance_sqr = global_position.distance_squared_to(target.global_position)
	
	if distance_sqr > follow_distance * follow_distance:
		global_position = global_position.lerp(target.global_position,delta * speed)
		
		if distance_sqr < snap_distance:
